...
int unlz77wii_raw20(unsigned char *in, int zsize, unsigned char *out, int size) {
    nintendo_ds_set_inout(in, zsize, out, size); HUF_Decode(""); size = nintendo_ds_set_inout(NULL, 0, NULL, 0);
    return size;
}
...
int unlz77wii(unsigned char *in, int insz, u8 **ret_out, int *full_outsz) {
    ...
    *ret_out = out;
    ...
    switch(tag >> 4) {
        ...
        case (WII_HUFF_TAG >> 4):  outsz = unlz77wii_raw20(in, insz, out, outsz);  break;
