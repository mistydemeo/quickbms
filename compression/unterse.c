/*
IBM TERSE decompression (PACK/SPACK) 0.1
by Luigi Auriemma
...
int terse_decompress(
    ...
    signed short    x414130 = 1;
    //signed short    x414134 = 1 or 0x10;
    signed short    x414138 = 0x80;
    signed short    x41413c = 1;    // 0:PACK 1:SPACK
    signed short    x414140 = 1;
    signed short    x414144 = 1;
    //signed short    x414150 = 1;
    signed short    x414154 = 0x100;
    signed short    x4522f8 = 0;
    ...
    void terse_decompress_dump(int dump) {
        signed short bx;
        for(bx = 8; bx > 0;) {
            if((bx <= 7) || (x414138 != 0x80)) {
                if(x414138 == 1) {
                    if(o < outl) *o++ = x4141b8[bx] & dump;
                    x414138 = 0x80;
                } else {
                    x414138 >>= 1;
                }
    ...
    void terse_decompress_out_crlf(void) {
        int     x;
        for(x = 0; x4156dc[x]; x++) terse_decompress_dump(x4156dc[x]);
    }

    void terse_decompress_out_code(int code) {
        if(code == 0) {
            if(!x414130) return;
            if(!x414140) return;
            if(!x414144) return;
            terse_decompress_out_crlf();
        } else {
            if(!x414130 || !x414140) {
                if(code >= 257) return;
                terse_decompress_dump(code - 1);
            } else if(!x414144) {
                terse_decompress_dump(x41a300[code - 1]);
                x4522f8++;
                if(x4522f8 == x414154) {
                    terse_decompress_out_crlf();
                    x4522f8 = 0;
    ...
    void terse_decompress_out(int t) {
    ...
    if(x41413c) {
        ...
        while(_t209) {
            ...
            terse_decompress_out(_t209);
            __esp20 = _t153;
            while(_t197) {
                __esp16 = x43e880[_t197];
                terse_decompress_out(x43c880[_t197]);
                x43e880[_t197] = _t209;
                _t209 = _t197;
                _t197 = __esp16;
            }
