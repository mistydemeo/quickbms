...
redo:
            ...
            if(argc >= 2) CMD.var[2] = parse_bms_add_var(3);    // names file
            if(argc >= 3) CMD.var[3] = parse_bms_add_var(4);    // type
            if(argc >= 4) CSTRING(4, ARG[5])                    // key
            if(argc >= 5) CSTRING(5, ARG[6])                    // ivec
            ...
            */
            ...
            if(ARG[4] && !g_filter_files) {
                mystrdup(&tmp, ARG[4]);
                g_filter_files = build_filter(tmp);
                FREE(tmp)
            }
    ...
    if(g_script_uses_append && !is_append_hashing_necessary) g_script_uses_append = 0;
    return(cmd);
}
