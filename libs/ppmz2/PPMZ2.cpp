...
EXPORT Ppmz2::Ppmz* Ppmz2::Ppmz::Create(int order, int trieMegs, int detMegs, CodingOptions options, /*LocalOrderEstimation::LOEType*/ int loeType, void (*loggingCallback)(const std::string&))
{
...
EXPORT bool Ppmz2::Ppmz::DecodeArraySub(unsigned char* rawBuf, unsigned int rawLength, unsigned char* compBuf)
{
...
EXPORT unsigned int Ppmz2::Ppmz::EncodeArraySub(unsigned char* rawBuf, unsigned int rawLen, unsigned char* compBuf)
{
	CodingMetrics metrics = {0};
...
__int64 EXPORT Ppmz2::Ppmz::Update(Context** contexts, int sym, unsigned long cntx, int order)
{
	Stopwatch timer;

	if ( ! (_options & CodingOptions__NoUpdate) )
	{
...
EXPORT __int64 Ppmz2::Ppmz::EncodeOrderMinusOne(int sym)
{
	Stopwatch timer;
	// encode raw with order -1
	if ( _options & CodingOptions__TextMode )
...
EXPORT int Ppmz2::Ppmz::DecodeOrderMinusOne(__int64* minusOneOrderTime)
{
	Stopwatch timer;
...
EXPORT int Ppmz2::Ppmz::EncodeFromOrder(Context** contexts, int sym, unsigned long cntx, bool* useFull, CodingMetrics* metrics)
{
	Stopwatch timer;
...
EXPORT int Ppmz2::Ppmz::DecodeFromOrder(Context** contexts, unsigned long cntx, bool* useFull, CodingMetrics* metrics, int* codedOrder)
{
	Stopwatch timer;
	// do the initial LOE to pick a start order
...
EXPORT bool Ppmz2::Ppmz::DetEncode(unsigned char* rawPtr, unsigned char* rawBuf, int sym, Context** contexts, bool* useFull, __int64* codingTime)
{
	Stopwatch timer;
...
EXPORT bool Ppmz2::Ppmz::DetDecode(unsigned char* rawPtr, unsigned char* rawBuf, int* sym, Context** contexts, bool* useFull, __int64* codingTime)
{
	Stopwatch timer;
...
EXPORT __int64 Ppmz2::Ppmz::DetUpdate(unsigned char* rawPtr, unsigned char* rawBuf, int sym)
{
	Stopwatch timer;
	if ( ! (_options & CodingOptions__NoUpdate) )
	{
...
EXPORT unsigned int Ppmz2::Ppmz::DecodeOrderMinusOneText()
{
...
EXPORT void Ppmz2::Ppmz::EncodeOrderMinusOne(unsigned int sym, unsigned int numChars)
{
    assert( ! _exclude->IsExcluded(sym) );
...
EXPORT void Ppmz2::Ppmz::EncodeOrderMinusOneText(unsigned int sym)
{
...
EXPORT unsigned int Ppmz2::Ppmz::DecodeOrderMinusOne(unsigned int numChars)
{
    unsigned int tot = 0;
...
EXPORT bool Ppmz2::Ppmz::EncodeFromContext(Context* cntx, unsigned long index, int sym, bool* pUseFull)
{
    int low,high,tot;
    int largest,escape;            
    SeeState *ss;
...
EXPORT bool Ppmz2::Ppmz::DecodeFromContext(Context* cntx, unsigned long index, int *psym, bool * pUseFull)
{
...
EXPORT void Ppmz2::Ppmz::ReInitCoder()
{
